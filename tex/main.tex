...
\noindent If $\partial \Omega$ is stricly convex and $c_1 = c_2 = 1$, the problem \eqref{eq:continuousProblem} is Lipschitz stable if the set $\omega_T \subset Q$ fulfills the geometric control condition (GCC) \cite{BLR88,BLR92}. More precisely, we have the following theorem, cf. \cite[Rem. A.5]{BFMO21control}. 

\begin{thm}
    Assume that $c_1= c_2 = 1$ and that $\partial \Omega$ is stricly convex. If $\omega_T \subset Q$ fulfills the GCC, then there exists a constant $C > 0$ such that for any $\phi \in H^1(Q)$ we have the following estimate 
    \begin{equation*}
...
In that case, this estimate can be applied to $\phi = u-u_h$ to recover convergence rates in terms of the norm on the left-hand side. However, in the general case where $c_1 \not = c_2 \not = 1$, the above result is not applicable. Let $\operatorname{dist}_c(x,\omega_T) := \inf_{y \in \omega_T} d_c(x,y)$, where $d_c$ is the metric induced by the wave speed $c$. Then, we have the following result \cite[Thm. 1.1]{Filippas22}: 
\begin{thm}[Thm. 1.1 of \cite{Filippas22}]\label{thm:filippas}
    For any nonempty subset $\omega_T \subset \Omega$ and $T > 2 \sup_{x \in \Omega} \operatorname{dist}_{c}(x,\omega_T)$, there exists $C,\kappa$ such that for any initial data $(u_0,u_1) \in H^1_0(\Omega) \times L^2(\Omega)$ and $u$ solving... one has for any $\mu > 0$ that 
    \begin{equation}
...
\begin{proof}
    ...
    \end{align*}
    Thus it follows by definition of the stabilizers and $\tnorm{(\Uh,\Zh)} = 0$ that $\Vert \wop \ul_1 \Vert_{H^{-1}(Q)} = 0$. It directly follows from Thm. \ref{thm:filippas} that the initial data is zero. {\color{gray} That means that $\ul_1$ solves the wave equation with rhs zero, zero initial data, is zero on the boundary and zero on $\omega$... should enough to say that $\ul_1 = 0$ everywhere!?}
\end{proof}
