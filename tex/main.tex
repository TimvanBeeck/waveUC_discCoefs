...
To this end, we follow reference \cite{Filippas23} in which it has been shown that under certain assumptions on the contrast of the sound speed and in a certain geometric setting so called whispering gallery modes exist. These are solutions of the wave equation that are exponentially decaying away from the interface and which show that the logarithmic stability estimate \eqref{eq:FilippasEstimateLog} from \cite{Filippas22} is in general optimal.  \par 
Indeed, in \cite{Filippas23} a sequence of eigenfunctions $w_{\ell}$  of $-\Delta_c$ with corresponding eigenvalues $\lambda_{\ell} \rightarrow \infty$ in a certain domain $\Omega$ has been constructed 
such that for all $\omega \subset \Omega$ with $\dis(\bar{\omega},\Gamma) > 0$ there exists constants $C,D > 0$ such that  
\[
\norm{ w_{\ell} }_{ \omega } \leq C \exp(-D \sqrt{ \lambda_{\ell } }  ), \quad 
- \Delta_c w_{\ell } = \lambda_{\ell} w_{\ell }, \quad 
w_n|_{\partial \Omega } = 0, \quad
\norm{ w_{\ell }  }_{ \Omega } = 1.  
\]
...
\[
\Vert  u_{\ell} \Vert_{L^2(\omega_T)} + \Vert u_{\ell} \Vert_{L^2(\Sigma)} + \Vert \square_c u_{\ell} \Vert_{H^{-1}(Q)}  = C_T \exp(-D \sqrt{ \lambda_{\ell } }  )
\]
goes to zero as $\ell \rightarrow \infty$. \par 
As shown in \cite[Section 2]{Filippas23}, a possible geometrical setup is an annulus where $\Omega_1 = {  R_0 < r < R_1 } $ and $\Omega_1 = {  R_1 < r < R_2 } $ with 
$R_0 < R_1 < R_2$. To ensure the existance of whispering Gallery modes the contrast has to fulfill the condition 
\[
\frac{c_1}{R_1} < \frac{c_2}{R_2}.
\]
Thanks to annulus geometry, the mode $w_{\ell}$ can be obtained by separating variables 
...
For fixed $\ell$ there exists many eigenpairs $( \lambda_{\ell}, \psi_{\ell} )$ solving \eqref{eq:eigval-prob-r} of which it sufficices however to pick just one for 
...
The corresponding solutions of the wave equation $u_{\ell}(t,x)$ as defined in \cref{eq:whispering-gallery-mode} are also displayed at the final time $t=T$ for 
$T=1$. 
The corresponding eigenvalues are $\lambda_0 \approx 190.669$ and $\lambda_1 \approx 404.347$. Since $\lambda_1 >  \lambda_0$ the corresponding 
...
\caption{ The upper panel displays the modes $\psi(r)$ for $\ell=1$ (left), respectively $\ell=10$ (right) obtained by solving the eigenvalue problem \cref{eq:eigval-prob-r}. 
	 ...
	 The cases where the data is given in $\omega_i$ is shown in magenta (solid lines with crosses) whereas the case where data is given in $\omega_o$ is displayed in cyan (same linestyle).
...
i.e. the convergence is as good as for a well-posed problem. However, when the data is given far away from the interface in $\omega_0$ then 
...
stability result \cref{assum:LipschitzStability} cannot hold for $\dis(\bar{\omega}_0,\Gamma) > 0$. The stability is of logarithmic type as asserted by 
