...
\noindent We define two subdomains $\Omega_1 = (0,0.5)$ and $\Omega_2 = (0.5,1.0)$ such that $\bar{\Omega} = \overline{\Omega}_1 \cup \overline{\Omega}_2$ and fix the wavespeed $c_2 := c \vert_{\Omega_2} = 1$ in $\Omega_2$. For variable $c_1 := c \vert_{\Omega_1}$, we consider the exact solution \cite{MHI08}
...
\noindent \Cref{fig:jumpCoefs:contrast:2.5} shows that the errors $\Vert u-L_{\Delta t} \ul_1 \Vert_{L^\infty(0,T;L^2(\Omega_R))}$ and $\Vert \partial_t (u-L_{\Delta t} \ul_1) \Vert_{L^2(0,T;L^2(\Omega_R))}$ converge with order $\mathcal{O}(h^k)$ for $T$ large enough, but if $T$ is chosen too small, the order of convergence degrades.
...
\begin{figure}[!htbp]
    ...
    \end{center}
    \caption{For the approximation of \eqref{eq:1D:exact:simple} with polynomial degree $k \in \{2,3\}$ and contrast $c_1 = 2.5$ we measure the errors $\Vert u-L_{\Delta t} \ul_1 \Vert_{L^\infty(0,T;L^2(\Omega_R))}$ (left) and $\Vert \partial_t (u-L_{\Delta t} \ul_1) \Vert_{L^2(0,T;L^2(\Omega_R))}$ (right) and compare to the respective error of the $L^2$-bestapproximation (dashed). We consider the final time $T = 0.5$ (top) and $T = 0.1$ (bottom). The gray dashed lines indicate the observed convergence rates. \textbf{adjust log lines..., shared legend...}}
    \label{fig:jumpCoefs:contrast:2.5}
  \end{figure}
...
%old figure
  ...
  \begin{figure}[!htbp]
    \begin{center}
        ...
        \pgfplotslegendfromname{named}
    \end{center}
    \caption{Exact solution \eqref{eq:1D:exact:simple} and approximations with $k = 3$ on refinement levels $L \in \{1,2,3,4 \}$ for contrasts $c_1 = 2.5$ (left) and $c_1= 5.5$ (right). The scale in the right plot indicates the mesh size $h = 1/2^{L+1}$ for the different refinement levels.}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.72]
            ...
            %ymin = 1e-3, ymax = 0.5e1,
                ...
                \addplot+[discard if not={L}{4},discard if not={contrast}{2.5},line width=3pt,mark=None,color=black] table [x=x, y=y, col sep=comma] {../dataNew/plots/simpleExact_ApproxPlot_T0.5.csv};
                ...
                \addplot+[discard if not={L}{4},discard if not={contrast}{5.5},line width=3pt,mark=None,color=black] table [x=x, y=y, col sep=comma] {../dataNew/plots/simpleExact_ApproxPlot_T0.5.csv};
        ...
        \pgfplotslegendfromname{named}
    \end{center}
    \caption{Exact solution \eqref{eq:1D:exact:simple} and approximations with $k = 3$ on refinement levels $L \in \{1,2,3,4 \}$ for contrasts $c_1 = 2.5$ (left) and $c_1= 5.5$ (right). The scale in the right plot indicates the mesh size $h = 1/2^{L+1}$ for the different refinement levels.v \textbf{generate the data for the exact plots, at which time are we?}}
    \label{fig:jumpCoefs:quality}
  \end{figure}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.72]
            ...
            ymin = -0.6e0, ymax = 0.6e0,
                ...
                \addplot+[discard if not={L}{1},discard if not={contrast}{7.5},line width=3.0pt,mark=None,color=yellow] table [x=x, y=y, col sep=comma] {../dataNew/plots/MultipleJumpsExact_ApproxPlot_T0.5.csv};
...
\noindent In this section, we present a numerical example in two space dimensions. We consider the unit-square $\Omega \coloneqq [0,1]^2 \subset \mathbb{R}^2$ partitioned into subdomains $\Omega_1 = (0.0,0.5)^2$ and $\Omega_2 = (0.5,1.0)^2$. 
