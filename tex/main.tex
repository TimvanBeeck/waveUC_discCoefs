...
\noindent If $\partial \Omega$ is stricly convex and $c_i = 1$, $i = 1,2$, the problem \eqref{eq:waveEquation}-\eqref{eq:dataMatch} is Lipschitz stable if the set $\omega_T \subset Q$ fulfills the geometric control condition (GCC) \cite{BLR88,BLR92}. More precisely, we have the following theorem, cf. \cite[Thm. A.4 \& Rem. A.5]{BFMO21control}. 
...
\begin{thm}\label{thm:Lipschitz}
    ...
    \begin{align*}
        ...
        \Vert \phi \vert_{t = 0} \Vert_{L^2(\Omega)} + \Vert \dt \phi \vert_{t = 0} \Vert_{H^{-1}(\Omega)} &\le C \left(\Vert \phi \Vert_{L^2(\omega_T)} + \Vert \phi \Vert_{L^2(\Sigma)} + \Vert \wop \phi \Vert_{H^{-1}(Q)} \right). 
    \end{align*}
\end{thm}
...
To approximate solutions of \eqref{eq:continuousProblem}, we search for stationary points of the Lagrangian 
\begin{align*}
...
With those terms, we define the primal stabilizer $S_h$ as 
\begin{equation}
...
\section{Analysis}
\noindent In this section, we analyze the approximation of the continuous problem \eqref{eq:continuousProblem} through stationary points of the Laplacian $\cL_h$. In particular, we show that the error between the first component $\ul_1$ of the discrete solution and the continuous solution $u$ is bounded by the best approximation error in suitable discrete norms defined below. Let $\vert \cdot \vert_{S_h}, \vert \cdot \vert_{\uparrow \downarrow}$, and $\Vert \cdot \Vert_{S_h^{\ast}}$ be the semi-norms, respectively norms, induced by the stabilizers $S_h$, $\Sud$, and $S_h^{\ast}$:
\begin{equation}
...
\begin{equation}
    \Vert (u - \ul_1) \vert_{t = 0} \Vert_{L^2(\Omega)}  + \Vert \dt (u - \ul_1) \vert_{t = 0} \Vert_{H^{-1}(\Omega)} \le C C_{0,1} \log(1 + C_{0,1} h^{-s} \Vert u \Vert_{H^m(Q)}^{-1}),
\end{equation} 
...
\begin{equation}\label{eq:1D:exact:simple}
    ...
    \end{cases}
\end{equation}
where we set $w_1 = 3 \pi$ and $w_2 = w_1 c_1 / c_2$. Furthermore, we consider the data domain $\omega = [0.0,0.25] \cup [0.75,1.0]$ and set $\Omega_R \coloneqq \Omega \setminus \omega = [0.25,0.75]$. We note that in one space dimension, the requirement that the domain $\omega_T$ fulfills the GCC is equivalent to the requirement $T > 2 \sup_{x \in \Omega} \operatorname{dist}_c(x,\omega_T)$ from Thm. \ref{thm:filippas}. In the following, we denote by $d(x,y) \coloneqq \vert x - y \vert$, $x, y \in \Omega$, the Euclidean metric and set 
\begin{equation}
    d_c(x,y) \coloneqq \mathbbm{1}_{\Omega_1} c_1(x)^{-1} d(x,y) + \mathbbm{1}_{\Omega_2} c_2(x)^{-1} d(x,y), \quad x,y \in \Omega.
\end{equation} 
Thus, in the case where $c_2 = 1$ is fixed, we require $T > 0.25(1+c_1^{-1})$. 
...
\begin{figure}[!htbp]
    \begin{center}
        \begin{tikzpicture}[scale=0.72]
            ...
            %L2-L2-u_t (k = 2)
            \addplot+[discard if not={order}{2}, discard if not={contrast}{2.5},line width=1.1pt,color=teal,mark=*] table [x=L, y=L2-L2-error-u_t, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.5_1D_jumpingCoefs_k3.csv};
            \addplot+[discard if not={order}{2}, discard if not={contrast}{2.5},line width=1.1pt,dashed,color=teal,mark=*] table [x=L, y=bestapprox-L2-L2-error-u_t, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.5_1D_jumpingCoefs_k3.csv};
            

            %L2-L2-u_t (k = 3)
            \addplot+[discard if not={order}{3}, discard if not={contrast}{2.5},line width=1.1pt,color=red,mark=star] table [x=L, y=L2-L2-error-u_t, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.5_1D_jumpingCoefs_k3.csv};
            \addplot+[discard if not={order}{3}, discard if not={contrast}{2.5},line width=1.1pt,dashed,color=red,mark=star] table [x=L, y=bestapprox-L2-L2-error-u_t, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.5_1D_jumpingCoefs_k3.csv};
            ...
            %L2-L2-u_t (k = 2)
            \addplot+[discard if not={order}{2}, discard if not={contrast}{2.5},line width=1.1pt,color=teal,mark=*] table [x=L, y=L2-L2-error-u_t, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.1_1D_jumpingCoefs_k3.csv};
            \addplot+[discard if not={order}{2}, discard if not={contrast}{2.5},line width=1.1pt,dashed,color=teal,mark=*] table [x=L, y=bestapprox-L2-L2-error-u_t, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.1_1D_jumpingCoefs_k3.csv};
            

            %L2-L2-u_t (k = 3)
            \addplot+[discard if not={order}{3}, discard if not={contrast}{2.5},line width=1.1pt,color=red,mark=star] table [x=L, y=L2-L2-error-u_t, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.1_1D_jumpingCoefs_k3.csv};
            \addplot+[discard if not={order}{3}, discard if not={contrast}{2.5},line width=1.1pt,dashed,color=red,mark=star] table [x=L, y=bestapprox-L2-L2-error-u_t, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.1_1D_jumpingCoefs_k3.csv};
  ...
  \begin{figure}[!htbp]
    \begin{center}
        \begin{tikzpicture}[scale=0.85]
            ...
            \addplot+[line width=1.1pt,color=orange,mark=diamond*] table [x=contrast, y=L-infty-L2-error-u, col sep=comma]{../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.5_multipleContrasts_1D_jumpingCoefs_k3.csv};
            \addplot+[line width=1.1pt,color=orange,mark=diamond*,dashed] table [x=contrast, y=bestapprox-L-infty-L2-error-u, col sep=comma]{../../wave_data_assimilation/data/jumpCoefs/simpleExact_T0.5_multipleContrasts_1D_jumpingCoefs_k3.csv};
            ...
            \legend{$\Vert \partial_t (u - u_h) \Vert_{L^2(0,T;L^2(\Omega_R))}$,,$\Vert u - u_h \Vert_{L^\infty(0,T;L^2(\Omega_R))}$}
            \end{groupplot}
        \end{tikzpicture}
    \end{center}
    \caption{We consider $\Vert \dt (u-u_h) \Vert_{L^2(0,T;L^2(\Omega_R))}$ and $\Vert u - u_h \Vert_{L^\infty(0,T;L^2(\Omega_R))}$ for polynomial degree $k = 3$ on the refinement level $L = 3$ for increasing contrast $c_1 \in \{1.0,1.5,...,4.5\}$. Both errors increase with the same order of magnitude as the respective best approximation error (dashed lines).}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.72]
            ...
            ymin = -0.6e0, ymax = 0.6e0,
                ...
                \addplot+[line width=2.5pt,mark=None,color=black] table [x=x, y=y, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/exact_plot_data_1D_multipleJumps_contrast7.5_T0.5.csv};
                ...
                \addplot+[line width=1.0pt,mark=None,color=teal!55!white] table [x=x, y=y, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/multipleJumps_approx_plot_data_WPFalse_contrast7.5_k3_uniform_T0.5_L4.csv};
                ...
                \node [draw=none] at (axis description cs:0.7,0.65) {\color{gray} $\Gamma_2$};
                ...
                \addplot+[line width=2.0pt,mark=None,color=black] table [x=x, y=y, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/exact_plot_data_1D_multipleJumps_contrast11.5.csv};
                \addplot+[line width=1.0pt,mark=None,color=teal!55!white] table [x=x, y=y, col sep=comma] {../../wave_data_assimilation/data/jumpCoefs/multipleJumps_approx_plot_data_WPFalse_contrast11.5_k3_uniform_T0.5_L4.csv};
