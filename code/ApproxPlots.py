...
GCC = True #only controls wether data is set only on the lhs (False) or on both sides (True)
...
def Solve(ref_lvl,order,contrast,stabs,plot_sol=False,plot_error=False,T=0.5):
    t0 = 0
    #T = 0.1 #GCC True for 0.5, but what with 0.1??
    ...
    def sol(t,xu):
        return ufl.sin(xu[0]-t)

    def dt_sol(t,xu):
        return -ufl.cos(xu[0]-t)
    ...
    w1 = 3*np.pi
    def sol(t,xu):
        return new_refsol(xu=xu[0],t=t,c1=c1,c2=c2,w1=w1,pos=pos,dt=False)
    
    def dt_sol(t,xu):
        return new_refsol(xu=xu[0],t=t,c1=c1,c2=c2,w1=w1,pos=pos,dt=True)
