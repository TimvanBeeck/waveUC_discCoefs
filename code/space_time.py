        ...
        for n in range(self.N):
            ...
            for j in range(self.q+1):
                for k in range(self.q+1):
                    ...
                    # J(U_h,W_h) 
                    print("Hello from Space-time with normal jump \n \n \n \n \n") #not called...
    ...
    def SetupSlabMatrix(self,with_jumps=True): 
        ...
        for j in range(self.q+1):
            for k in range(self.q+1):
                ...
                #jump(grad(u),n)
                normal = self.n_facet
                a_pre += gamma_primal * delta_t * elmat_time["M_time_q_q"][k,j]  * 0.5*(h('+')+h('-'))*inner( jump(self.csq * grad(u1_p[j]),normal) , jump(self.csq * grad(w1_p[k]),normal) )  * dS
                # I_0(U_h,W_h)
